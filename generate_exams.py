...
themes = [
    {
        ...
        "replacements": {
            "学校": "公司", "学生": "员工", "老师": "经理", "校长": "董事长",
            "考试": "会议", "考": "开", "学习": "工作", "成绩": "业绩", "复习": "加班",
            "出国留学": "出国出差", "图书": "文件", "阅读": "审核", "毕业": "升职",
            "大学": "部门", "同学": "同事", "教育": "管理", "教室": "会议室"
        }
    },
    {
        "name": "travel",
        ...
        "replacements": {
            "学校": "机场", "学生": "旅客", "老师": "导游", "校长": "机长",
            "考试": "航班", "学习": "旅游", "成绩": "风景", "复习": "休息",
            "出国留学": "出国旅游", "图书": "护照", "阅读": "游览", "毕业": "到达",
            "大学": "航班", "同学": "游客", "教育": "服务", "教室": "候机室", "公司": "旅行社"
        }
    },
    {
        "name": "health",
        ...
        "replacements": {
            "学校": "医院", "学生": "病人", "老师": "医生", "校长": "院长",
            "考试": "检查", "学习": "治疗", "成绩": "健康", "复习": "吃药",
            "出国留学": "去大医院", "图书": "病历", "阅读": "休息", "毕业": "出院",
            "大学": "病房", "同学": "护士", "教育": "治疗", "教室": "手术室", "公司": "诊所"
        }
    },
    {
        "name": "shopping",
        ...
        "replacements": {
            "学校": "商场", "学生": "顾客", "老师": "店长", "校长": "老板",
            "考试": "活动", "学习": "购物", "成绩": "质量", "复习": "挑选",
            "出国留学": "买奢侈品", "图书": "商品", "阅读": "消费", "毕业": "结账",
            "大学": "超市", "同学": "买家", "教育": "打折", "教室": "专柜", "公司": "店铺"
        }
    }
]
...
def process_tf(q, theme):
    ...
    if random.choice([True, False]):
        if "不" in q["stmt"]:
            ...
            q["stmt"] = "不" + q["stmt"] if len(q["stmt"]) < 10 else q["stmt"].replace("是", "不是", 1)
...
def process_order(q, theme):
    q["sents"] = [apply_replacements(s, theme["replacements"]) for s in q.get("sents", [])]
    q["exp"] = apply_replacements(q.get("exp", ""), theme["replacements"])
...
def process_reorder(q, theme):
    if "words" in q:
        ...
        if isinstance(current_ans_str, str):
            q["ans"] = encode_ans(apply_replacements(current_ans_str, theme["replacements"]))
    q["exp"] = apply_replacements(q.get("exp", ""), theme["replacements"])
...
def main():
    ...
    for theme in themes:
        ...
        # Modify title
        new_data["page"]["title"] = apply_replacements(new_data["page"]["title"], theme["replacements"])
        new_data["page"]["subtitle"] = apply_replacements(new_data["page"]["subtitle"], theme["replacements"]) + f" ({theme['name']}版)"
        
        exam = new_data["examData"]
        
        for q in exam["listening"]["tf"]: process_tf(q, theme)
        for q in exam["listening"]["mc"]: process_mc(q, theme)
        for q in exam["listening"]["long"]: process_mc(q, theme)
        
        for q in exam["reading"]["fill"]: process_fill(q, theme, None)
        for q in exam["reading"]["order"]: process_order(q, theme)
        for q in exam["reading"]["comp"]: process_mc(q, theme)
        
        for q in exam["writing"]["reorder"]: process_reorder(q, theme)
        for q in exam["writing"]["sentence"]: process_sentence(q, theme)
